...
class Be(StochasticProcess):
    def __init__(
        self,
        diffusion_coefficient: real = 1.0,
    ):
        """
        Initialize a Brownian Excursion (Be) object.
        ...
        # start_position is implicitly 0 for Brownian Excursion
    ...
    def central_moment(
        ...
        if _order == 0:
            return 1.0
        # First central moment of an excursion might not be trivially zero depending on definition.
        # We rely on the _core implementation.

        return _core.be_central_moment(
            self.diffusion_coefficient,
            _duration,
            _step_size,
            _order,
    ...
    def fpt(
        ...
        step_size: real = 0.01,
        max_duration: real = 1000,  # This is the excursion's fixed duration
    ) -> Optional[float]:
        # For an excursion of fixed duration T, FPT within (0,T) to a level.
        # The domain should reflect levels, and max_duration is the excursion duration.
        ...
        return _core.be_fpt(
            ...
            _step_size,
            (_a, _b),
            _max_duration,  # This should be the fixed duration of the excursion
        )
    ...
    def fpt_raw_moment(
        ...
        particles: int,
        step_size: real = 0.01,
        max_duration: real = 1000,  # Excursion duration
    ) -> Optional[float]:
        _a, _b = validate_domain(domain, process_name="Be FPT raw moment")
        _order = validate_order(order)
        _particles = validate_particles(particles)
        _step_size = validate_positive_float_param(step_size, "step_size")
        _max_duration = validate_positive_float_param(
        ...
        return _core.be_fpt_raw_moment(
            self.diffusion_coefficient,
            (_a, _b),
            _order,
            _particles,
    ...
    def fpt_central_moment(
        ...
        _a, _b = validate_domain(domain, process_name="Be FPT central moment")
        _order = validate_order(order)
        _particles = validate_particles(particles)
        _step_size = validate_positive_float_param(step_size, "step_size")
        _max_duration = validate_positive_float_param(
        ...
        return _core.be_fpt_central_moment(
            self.diffusion_coefficient,
            (_a, _b),
            _order,
            _particles,
    ...
    def occupation_time(
        self,
        domain: tuple[real, real],
        duration: real,  # Excursion duration
        step_size: real = 0.01,
    ) -> float:
        _a, _b = validate_domain(domain, process_name="Be Occupation Time")
    ...
    def occupation_time_raw_moment(
        ...
        return _core.be_occupation_time_raw_moment(
            self.diffusion_coefficient,
            (_a, _b),
            _order,
            _particles,
    ...
    def occupation_time_central_moment(
        ...
        _a, _b = validate_domain(domain, process_name="Be Occupation central moment")
        _order = validate_order(order)
        _particles = validate_particles(particles)
        _duration = validate_positive_float_param(
            duration, "duration (excursion duration)"
        ...
        if _order == 0:
            return 1.0
        # if _order == 1: return 0.0 # Rely on core implementation

        return _core.be_occupation_time_central_moment(
            self.diffusion_coefficient,
            (_a, _b),
            _order,
            _particles,
